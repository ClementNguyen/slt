...
def validate_on_pose_data(
    ...
    """
    valid_iter = make_pose_data_iter(
        dataset=data,
...
def dope_test(
    ...
    # load the data
    _, dev_data, test_data, gls_vocab, txt_vocab = load_pose_data(data_cfg=cfg["data"])
    ...
    if do_recognition:
        ...
        for rbw in recognition_beam_sizes:
            ...
            dev_recognition_results[rbw] = validate_on_pose_data(
                model=model,
                data=dev_data,
                batch_size=batch_size,
                use_cuda=use_cuda,
    ...
    if do_translation:
        ...
        for tbw in translation_beam_sizes:
            ...
            for ta in translation_beam_alphas:
                dev_translation_results[tbw][ta] = validate_on_pose_data(
                    model=model,
                    data=dev_data,
                    batch_size=batch_size,
                    use_cuda=use_cuda,
    ...
    test_best_result = validate_on_pose_data(
        model=model,
        data=test_data,
        batch_size=batch_size,
        use_cuda=use_cuda,
