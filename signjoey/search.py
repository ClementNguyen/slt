...
def transformer_greedy_feat(
    ...
    # start with BOS-symbol for each sentence in the batch
    ys = encoder_body.new_full([batch_size, 1], bos_index, dtype=torch.long)
    ...
    for _ in range(max_output_length):
        ...
        with torch.no_grad():
            ...
            decoder_hand_1_out = decoder_hand(
            ...
            decoder_hand_2_out = decoder_hand(
            ...
            concat_decoder = torch.cat([decoder_body_out[1], decoder_face_out[1], decoder_hand_1_out[1], decoder_hand_2_out[1]], dim=2)
            logits = output_layer(concat_decoder)[:, -1]
            _, next_word = torch.max(logits, dim=1)
            next_word = next_word.data
            ys = torch.cat([ys, next_word.unsqueeze(-1)], dim=1)
...
#        greedy_fun = recurrent_greedy
    ...
    greedy_fun = transformer_greedy_feat
...
def beam_search_feat(
    ...
    # init
    transformer = isinstance(decoder_body, TransformerDecoder)
    batch_size = src_mask.size(0)
    att_vectors = None  # not used for Transformer
    ...
    # pylint: disable=protected-access
    if not transformer:
        hidden = decoder_body._init_hidden(encoder_body_hidden)
    else:
        hidden = None
    ...
    encoder_body = tile(
        encoder_body.contiguous(), size, dim=0
    )  # batch*k x src_len x enc_hidden_size
    encoder_face = tile(
        encoder_face.contiguous(), size, dim=0
    )  # batch*k x src_len x enc_hidden_size
    encoder_hand_1 = tile(
        encoder_hand_1.contiguous(), size, dim=0
    )  # batch*k x src_len x enc_hidden_size
    encoder_hand_2 = tile(
        encoder_hand_2.contiguous(), size, dim=0
    )  # batch*k x src_len x enc_hidden_size
    src_mask = tile(src_mask, size, dim=0)  # batch*k x 1 x src_len
    ...
    # numbering elements in the batch
    batch_offset = torch.arange(
        batch_size, dtype=torch.long, device=encoder_body.device
    )

    # numbering elements in the extended batch, i.e. beam size copies of each
    # batch element
    beam_offset = torch.arange(
        0, batch_size * size, step=size, dtype=torch.long, device=encoder_body.device
    )
    ...
    alive_seq = torch.full(
        ...
        bos_index,
        dtype=torch.long,
        device=encoder_body.device,
    )

    # Give full probability to the first beam on the first step.
    topk_log_probs = torch.zeros(batch_size, size, device=encoder_body.device)
    topk_log_probs[:, 1:] = float("-inf")
    ...
    for step in range(max_output_length):
        ...
        # concatenate decoder outputs
        concat_decoder = torch.cat([decoder_body_out[1], decoder_face_out[1], decoder_hand_1_out[1], decoder_hand_2_out[1]], dim=2)
        logits = output_layer(concat_decoder)
        ...
        # flatten log_probs into a list of possibilities
        curr_scores = curr_scores.reshape(-1, size * decoder_body.output_size)
